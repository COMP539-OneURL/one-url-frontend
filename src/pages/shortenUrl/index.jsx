import React, { useEffect } from "react";
import { useState } from "react";
import Layout from "../../components/layout";
import { Form, Input, Row, Col, Button, Typography, Carousel } from "antd";
import { CopyOutlined, CheckOutlined, FacebookOutlined, LinkedinOutlined, MailOutlined, TwitterOutlined } from "@ant-design/icons";
import { Link } from "react-router-dom";
import { EmailShareButton, FacebookShareButton, TwitterShareButton } from "react-share";
import axios from "axios";
// import B
const url = 'http://localhost:4567'

const { Item } = Form;
const { Title, Paragraph } = Typography;

const ShortenUrl = () => {
  const [result, setResult] = useState("https://oneurl.com/3tsasweds");
  const [ifCopy, setIfCopy] = useState(false);

  useEffect(()=>{
    const timer = setTimeout(()=>{
      setIfCopy(false)
    },1000)
    return () => clearTimeout(timer)
  }, [ifCopy])

  // useEffect(()=>{
  //   axios.post(`${url}/shorten`, article)
  //       .then(response => this.setState({ articleId: response.data.id }));
  // }, [ifCopy])

  const copy = () => {
    navigator.clipboard.writeText(result);
    setIfCopy(true);
  };

  const contentStyle = {
    height: "160px",
    color: "#fff",
    lineHeight: "160px",
    textAlign: "center",
    background: "#364d79",
  };

  const onFinish = (values) => {
    // todo
    const { originUrl } = values
    axios.post(`${url}/shorten?url=${originUrl}`)
      .then(response => {
        const {status, data} = response
        if (status === 200) {
          setResult(`http://localhost:4567/shortenRedirect/${data}`)
        }
      });
  };

  return (
    <Layout>
      <Form onFinish={onFinish}>
        <Row style={{ marginTop: "8vh" }}>
          <Col span={11} offset={6}>
            <Row>
              <Item
                name='originUrl'
                style={{ width: "85%", borderTopLeftRadius: "10px", borderBottomLeftRadius: "10px" }}
                rules={[{ required: true, message: "Please input your url!" }]}
              >
                <Input style={{ borderTopLeftRadius: "10px", borderBottomLeftRadius: "10px" }} size='large' />
              </Item>
              <Button
                type='primary'
                htmlType='submit'
                size='large'
                style={{
                  backgroundColor: "#8c8c8c",
                  borderColor: "#8c8c8c",
                  borderTopLeftRadius: "0px",
                  borderBottomLeftRadius: "0px",
                  borderTopRightRadius: "10px",
                  borderBottomRightRadius: "10px",
                  color: "white",
                  width: "15%",
                }}
              >
                shorten
              </Button>
            </Row>
          </Col>
        </Row>
        {/* <Row>
          <Col span={10} offset={6}>
            <Title level={3}>Customize Link Domain</Title>
          </Col>
        </Row> */}
        {/* <Row>
          <Col span={5} offset={6}>
            <Item name='customizeLinkDomain'>
              <Input style={{ borderRadius: "10px" }} size='large' />
            </Item>
          </Col>
        </Row> */}
        <Row>
          <Col span={9} offset={6} style={{ backgroundColor: "grey", paddingLeft: "10px" }}>
            <Row align='middle' style={{ height: "50px", color: "white", fontSize: "20px" }}>
              {result}
            </Row>
          </Col>
          <Col span={2}>
            <Col span={22} offset={2} style={{ backgroundColor: "grey", paddingLeft: "10px" }}>
              <Row align='middle' justify='center' style={{ height: "50px" }}>
                <Link to='#' onClick={copy} style={{ color: "white", fontSize: "20px" }}>
                  {ifCopy ? <CheckOutlined /> : <CopyOutlined />} Copy
                </Link>
              </Row>
            </Col>
          </Col>
        </Row>
        <Row style={{ marginTop: "20px" }}>
          {/* <Col span={12}> */}
            <Col span={11} offset={6} style={{ backgroundColor: "grey", paddingLeft: "10px" }}>
              <Row align='middle' style={{ height: "50px" }}>
                <Col span={2} offset={0}>
                  <Row style={{ color: "white", fontSize: "20px" }}>Shared</Row>
                </Col>
                <Col span={2} offset={2}>
                  <FacebookShareButton url={result} quote="oneUrl" hashtag="#shortenUrl" description="generated by oneUrl" style={{ color: "white", fontSize: "30px" }}>
                    <FacebookOutlined />
                  </FacebookShareButton>
                </Col>
                <Col span={2}>
                  <TwitterShareButton url={result} title="oneUrl" hashtags={["oneUrl"]} style={{ color: "white", fontSize: "30px" }}>
                    <TwitterOutlined />
                  </TwitterShareButton>
                </Col>
                <Col span={2}>
                  <EmailShareButton subject="oneUrl" body={result} style={{ color: "white", fontSize: "30px" }}>
                    <MailOutlined />
                  </EmailShareButton>
                </Col>
              </Row>
            </Col>
          {/* </Col> */}
        </Row>
        <Row style={{ marginTop: "20px" }}>
          <Col span={11} offset={6}>
            <Carousel autoplay>
              {/* https://ccd.rice.edu/employers/expos-and-recruiting-events/expo */}
              <div>
                <a href="https://ccd.rice.edu/employers/expos-and-recruiting-events/expo" target="_blank">
                  <img width="700" src={`${window.location.origin + '/beer_bike.png'}`} />
                </a>
                {/* <h3 style={contentStyle}>Ad1</h3> */}
              </div>
              {/* https://alumni.rice.edu/beerbike */}
              <div>
                <a href="https://alumni.rice.edu/beerbike" target="_blank">
                  <img width="700" src={`${window.location.origin + '/career_faire.png'}`} />
                </a>
              </div>
              {/* https://coffeehouse.rice.edu/ */}
              <div>
                <a href="https://coffeehouse.rice.edu" target="_blank">
                  <img width="700" src={`${window.location.origin + '/rice_coffee_house.png'}`} />
                </a>
              </div>
            </Carousel>
          </Col>
        </Row>
      </Form>
    </Layout>
  );
};

export default ShortenUrl;
